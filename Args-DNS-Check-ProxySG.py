...
# Imports Section
import subprocess, pexpect, os, stat, time, paramiko, sys
import pandas as pd
import numpy as np
...
def arg_dns_check(arg1, arg2, arg3, arg4):
    ...
    print(command)
    return entered_site, testsitename, host, command

# End of argument passing function

# Get data function from ProxySG
def getdata(host,username,password,cmd,entered_site,testsitename):
...
entered_site, testsitename, host, command = arg_dns_check(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4])

# Function to loop with command list
def cmdlist(command,host,username,password):
    if len(command) != 0:
        for cmd in command:
            getdata(host,username,password,cmd,entered_site,testsitename)
    else:
        print("Command length is 0 -ZERO. Please input atlease 1 command")
# End of command loop finction
