...
# Imports Section
import subprocess, pexpect, os, stat, time, paramiko, socket
import pandas as pd
import numpy as np
...
sitename = input(Fore.BLUE + "\nEnter the sitename(Ex - For Google Sites : ginmu1/gitmi11, for Azure Sites acnbj2/acnsh2) : ")
site_check(sitename)
site = (sitename+"-")
...
print (Fore.MAGENTA + "\nEstablishing SSH Connection to ProxySG and issuing certificate revoke command ...\n")
host =(username+"@"+ip)
...
# Send in ProxySG conf terminal command
child.expect('#')
child.sendline('conf terminal')
child.expect('\(config\)')

# Send in SSL elevated terminal command
child.sendline('ssl')
child.expect('\(config ssl\)')

# Send in intermediate-cert-cache elevated terminal command
child.sendline('intermediate-cert-cache')
print (child.after.decode())
child.expect('\(config ssl icc\)')
print (child.after.decode())

# Send in clear-cache command
child.sendline('clear-cache')
print (child.before.decode()+child.after.decode())
child.expect('\(config ssl icc\)')
print (child.before.decode()+child.after.decode())

# Send in exit from icc
child.sendline('exit')
print (child.after.decode())

# Send in exit from ssl
child.expect('\(config ssl\)')
print (child.after.decode())
child.sendline('exit')

# Send in exit from config-terminal
child.expect('\(config\)')
child.sendline('exit')

# Send in exit from ProxySG Console
child.expect('#')
child.sendline('exit')
...
message = "Cleared SSL CACHE in ProxySG"
